...
export const RouterCartItem = ({ product, taxRate, onQuantityChange, onRemove }) => {
  const t = useTranslations('cart');
  const commonT = useTranslations('common');

  // Use the same logic as productPricing.js to determine pricing type
  ...
  const setupPriceField = parseFloat(
    product.s_price ||
      ...
      product.rawProductData?.m_setup || // This is where the Router Wap Ax price is stored
      0
  );
  ...
  // Determine actual pricing based on paytype (like productPricing.js)
  let actualPrice = 0;
  let actualSetupPrice = 0;
  let isInstallmentPlan = false;

  if (isOneTimePurchaseByPaytype) {
    // For one-time purchases, check if price is in setup field (like Router Wap Ax)
              ...
              {displaySetupPrice > 0 && (
                <p
                  className={`text-xs text-secondary/80 ${
                    displayPrice > 0 ? 'mt-0.5' : 'text-sm font-medium text-secondary'
                  }`}
                >
                  {displayPrice > 0 ? '+ ' : ''}
