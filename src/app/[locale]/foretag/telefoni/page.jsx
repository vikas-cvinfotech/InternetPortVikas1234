...
import { featuresIncluded } from '@/components/telephony/telephonyData';
import { pricingData } from '@/components/pricingData';

export default function TelephonyPage() {
  const t = useTranslations('telephonyPage');
  const features = featuresIncluded(t);
  const costIcons = {
    openingFee: (
      <svg
        width="20"
  ...
  const pricingItems = t.raw('callCostsSection.pricingCard.items');
  const calculationRows = t.raw('callCostsSection.calculationCard.rows');
  const faqData = t.raw('faqSection.faqs');
  return (
    <div className="bg-primary text-secondary">
      <CommonBanner
        imageAlt="kvinna-telefon-skrivbord"
        imageSrc="https://internetportcom.b-cdn.net/se/img/kvinna-telefon-skrivbord.png"
        titlePart1={t('telephonyBanner.title')}
        desc={t('telephonyBanner.description')}
        link="/kategori/telefoni"
        linkLabel={t('telephonyBanner.linkLabel')}
      />
      <ContentBlock
      ...
      <div className="px-4 sm:px-[50px] xl:px-[80px] xxl:px-[135px] flex flex-col gap-30 pb-24 mb-1">
        <h2 className="text-3xl font-bold text-darkGray">{t('featuresSection.mainTitle')}</h2>
      ...
      <div className="relative">
        <div className="px-4 sm:px-[50px] xl:px-[80px] xxl:px-[135px] py-24 relative z-[1]">
          <div className="text-center font-bold mb-[60px] mt-1">
            <h2 className="text-[32px] mb-4">{t('callCostsSection.mainTitle')}</h2>
            <p className="text-base font-normal text-paraSecondary">
              {t('callCostsSection.subTitle')}
            </p>
          </div>
                ...
                <div className="border rounded-lg shadow-sm bg-primary p-8">
                  <div className="flex justify-between items-center mb-16">
                    <h2 className="text-2xl font-bold text-paraSecondary">
                      {t('callCostsSection.pricingCard.title')}
                    </h2>
                    <Link
                      href="javascript:void(0)"
                      className="text-base font-normal bg-lightgreen rounded-lg px-3 py-1 leading-[24px] text-primary capitalize inline-block"
                    >
                      {t('callCostsSection.pricingCard.badge')}
                    </Link>
                  </div>
                  <div className="flex flex-col gap-4">
                    {Object.entries(pricingItems).map(([key, item]) => (
                      <div key={key} className="flex justify-between items-center gap-2">
                        <div className="flex items-center gap-2">
                          <div className="flex-shrink-0">{costIcons[key]}</div>
                          <span className="text-paraSecondary text-base font-normal">
                            {item.label}
                          </span>
                        </div>
                        <div className="text-accent font-bold text-base">{item.price}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="border rounded-lg shadow-sm bg-primary p-8">
                  <div className="flex justify-between items-center mb-16">
                    <h2 className="text-2xl font-bold text-paraSecondary text-center md:text-start">
                      {t('callCostsSection.calculationCard.title')}
                    </h2>
                  </div>
                  <p className="text-base font-regular text-paraSecondary mb-8 text-center md:text-start">
                    {t('callCostsSection.calculationCard.description')}
                  </p>
                  <div className="flex flex-col">
                    {Object.entries(calculationRows).map(([key, row]) => (
                      <div
                        key={key}
                        className="flex flex-col md:flex-row justify-between items-center py-[12px] border-b border-secondaryBg"
                      >
                        <div className="flex flex-col md:flex-row items-center gap-2 text-paraSecondary text-base font-normal">
                          ...
                          <span>{row.calculation}</span>
                        </div>
                        <div className="text-paraSecondary font-normal text-base">{row.result}</div>
                      </div>
                    ))}
                  </div>
                  <div className="flex justify-between items-center text-base font-bold mt-8">
                    <div className="text-secondary">
                      {t('callCostsSection.calculationCard.total.label')}
                    </div>
                    <div className="text-accent">
                      {t('callCostsSection.calculationCard.total.price')}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="px-4 sm:px-[50px] xl:px-[80px] xxl:px-[135px]">
        <div className="text-center font-bold mb-[60px] mt-1">
          <h2 className="text-[32px] mb-4">{t('internationalPrices.title')}</h2>
          <p className="text-base font-normal text-paraSecondary">
            {t('internationalPrices.description')}
          </p>
        </div>
