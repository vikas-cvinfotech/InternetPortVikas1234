...
export default function BredbandPageClient({ locale }) {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        const routerProductIds = process.env.NEXT_PUBLIC_ROUTER_PRODUCT_IDS || '';

        const response = await fetch(
          `/api/hostbill/get-products-by-ids?ids=${routerProductIds}&language_id=${languageId}&category=bredband`
        );
        ...
        console.error('Error fetching broadband products:', err);
  ...
  return (
          ...
          <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
            <div className="pb-6 border-b border-secondary/10">
              <h1 className="text-3xl font-bold tracking-tight text-secondary sm:text-4xl">
                {t('categoriesPage.categories.broadband.name')}
              </h1>
              <p className="mt-4 text-base text-secondary/70">
                {t('categoryPages.broadband.subtitle')}
              </p>
            </div>
    ...
    >
      <ListingPageLayout locale={locale}>
        <ListingHeader
          title={t('categoriesPage.categories.broadband.name')}
          subtitle={t('categoryPages.broadband.subtitle')}
