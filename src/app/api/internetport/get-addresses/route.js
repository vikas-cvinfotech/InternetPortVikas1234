...
export async function POST(req) {
  ...
  const rateLimitResponse = await applyRouteRateLimit(req, generalRateLimiter);
  ...
  try {
    ...
    if (!apiKey) {
      console.error('REST_API_2_KEY environment variable is not set.');
      return NextResponse.json(
    ...
    const externalApiUrl = process.env.REST_API_2_ENDPOINT;
    ...
    const externalApiResponse = await fetch(`${externalApiUrl}/services/`, {
      method: 'POST',
      headers: {
