...
export async function POST(req) {
  ...
  const generalLimit = await applyRouteRateLimit(req, generalRateLimiter);
  if (generalLimit) return generalLimit;

  // Layer 2: Support ticket rate limiting (10 req/2min per IP)
  const authLimit = await applyRouteRateLimit(req, authRateLimiter);
