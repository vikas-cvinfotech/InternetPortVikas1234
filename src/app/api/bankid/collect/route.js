...
import { generalRateLimiter, pollingRateLimiter, applyRouteRateLimit } from '@/lib/rateLimit';
...
export async function POST(request) {
  ...
  // Layer 2: Polling rate limiting (300 req/5min per IP)
  const pollingLimit = await applyRouteRateLimit(request, pollingRateLimiter);
  if (pollingLimit) return pollingLimit;
