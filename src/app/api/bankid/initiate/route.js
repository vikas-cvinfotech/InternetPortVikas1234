...
import { generalRateLimiter, authRateLimiter, applyRouteRateLimit, applyRateLimiter, getClientIp } from '@/lib/rateLimit';
...
export async function POST(request) {
  ...
  // Layer 2: BankID auth rate limiting (40 req/2min per IP) - disabled in development due to React Strict Mode
  if (process.env.NODE_ENV === 'production') {
    const { success } = await applyRateLimiter(authRateLimiter, ip);
    if (!success) {
      // Log rate limit events for monitoring BankID authentication abuse
      console.warn('[BANKID INITIATE] Rate limit exceeded for IP:', ip);
